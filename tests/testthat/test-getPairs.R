...
test_that("getPairs throws error for invalid pair input", {
  expect_error(getPairs(123), "Invalid input: 'pairs' must be a character vector with at least one element.")
})

## Test for Invalid Info Parameter
test_that("getPairs throws error for invalid info parameter", {
  expect_error(getPairs("ADAEUR", info = "invalid_info"), "Invalid 'info' parameter.")
})

## Test for Invalid Country Code
test_that("getPairs throws error for invalid country code input", {
  expect_error(getPairs("ADAEUR", country_code = 123), "Invalid input: 'country_code' must be a single character string or NULL.")
})
