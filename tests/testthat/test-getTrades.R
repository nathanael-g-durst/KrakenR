...
test_that("getTrades returns data frame for a valid pair", {
  ...
  expect_true(all(c("Price", "Volume", "Time", "Order_Type", "Execution_Type", "Miscellaneous", "Trade_ID") %in% colnames(result)))
})
...
test_that("getTrades returns correct number of trades when count is specified", {
...
test_that("getTrades throws error for invalid pair input", {
  expect_error(getTrades(123), "Invalid input: 'pair' must be a single character string.")
})

## Test for Invalid 'since' Parameter
test_that("getTrades throws error for invalid 'since' format", {
  expect_error(getTrades("XTZUSD", since = "invalid_date"), "Invalid 'since' format.")
})

## Test for Invalid 'count' Parameter
test_that("getTrades throws error for invalid 'count' parameter", {
  expect_error(getTrades("XTZUSD", count = 2000), "Invalid input: 'count' must be a number between 1 and 1000.")
})
