...
#'
#' @param assets A character vector specifying the assets to retrieve. Use "All" to fetch data for all assets.
#'               For specific assets, provide their abbreviations (e.g., "ADA" or c("BTC", "ETH")). Default is "All".
#'
#' @return A data frame containing detailed information about the requested assets.
#' @export
...
getAssets <- function(assets = "All") {
  ...
  if (!is.character(assets) || length(assets) < 1) {
    stop("Invalid input: 'assets' must be a character vector with at least one element.")
  }
  ...
  for (asset in names(assetsList)) {
    asset_data <- tryCatch({
      ...
      warning(paste("Failed to process asset:", asset, "with error:", e$message))
